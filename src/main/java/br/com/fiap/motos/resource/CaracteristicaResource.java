...
public class CaracteristicaResource implements ResourceDTO<Caracteristica, CaracteristicaRequest, CaracteristicaResponse> {
    ...
    public ResponseEntity<Collection<CaracteristicaResponse>> findAll(
        ...
        var encontrados = caracteristicaService.findAll(example);
        ...
        var resposta = encontrados.stream()
                .map(caracteristicaService::toResponse)
                .toList();
    ...
    public ResponseEntity<CaracteristicaResponse> findById(@PathVariable Long id) {
        var encontrado = caracteristicaService.findById(id);
        if (encontrado == null) {
            return ResponseEntity.notFound().build();
        }
        var resposta = caracteristicaService.toResponse(encontrado);
        return ResponseEntity.ok(resposta);
    }
    ...
    @PostMapping
    @Transactional
    public ResponseEntity<CaracteristicaResponse> save(@RequestBody @Valid CaracteristicaRequest request) {
        var entity = caracteristicaService.toEntity(request);
        var saved = caracteristicaService.save(entity);
        var resposta = caracteristicaService.toResponse(saved);
