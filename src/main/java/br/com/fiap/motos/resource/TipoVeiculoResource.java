...
@RestController
@RequestMapping(value = "/tipos-de-veiculo")
public class TipoVeiculoResource implements ResourceDTO<TipoVeiculo, TipoVeiculoRequest, TipoVeiculoResponse> {
    ...
    public ResponseEntity<Collection<TipoVeiculoResponse>> findAll(
        ...
        var encontrados = tipoVeiculoService.findAll(example);
    ...
    public ResponseEntity<TipoVeiculoResponse> findById(@PathVariable Long id) {
        var encontrado = tipoVeiculoService.findById(id);
        if (encontrado == null) {
            return ResponseEntity.notFound().build();
        }
        var resposta = tipoVeiculoService.toResponse(encontrado);
        return ResponseEntity.ok(resposta);
    }
    ...
    @PostMapping
    @Transactional
    public ResponseEntity<TipoVeiculoResponse> save(@RequestBody @Valid TipoVeiculoRequest request) {
        var entity = tipoVeiculoService.toEntity(request);
        var saved = tipoVeiculoService.save(entity);
        var resposta = tipoVeiculoService.toResponse(saved);
